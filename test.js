...
test('replacement', function (t) {
  ...
  t.ok(result.shader, 'shader defined')
  t.equal(result.shader._fref.src.indexOf('test[10]'), 36, 'line found')
  t.deepEqual(result.defaults.color, [0, 100, 0])
  t.end()
})
...
test('multiple replacement', function (t) {
  ...
  var result = material(gl, flat, {'COUNT': 10})
  t.ok(result.shader, 'shader defined')
  t.equal(result.shader._fref.src.indexOf('a[10]'), 36, 'line found')
  t.equal(result.shader._fref.src.indexOf('b[10]'), 56, 'line found')
  t.deepEqual(result.defaults.color, [0, 100, 0])
  t.end()
})